...
function withLock(lockPath, fn, lockMessage) {
  try {
...
function sh(cmd, cwd = NEBULA_ROOT) {
  try {
    return {
      ok: true,
...
function alignmentForTask(taskId) {
  const map = {
...
function loadQueue() {
  const fallback = {version: 1, items: [], dead_letter: []};
  const queue = readJson(QUEUE_PATH, fallback);
...
function upsertQueueTasks(backlog, nowIso) {
  return withLock(QUEUE_LOCK_PATH, () => {
    const queue = loadQueue();
    const seenIds = new Set([
      ...queue.items.map((x) => x.id),
      ...queue.dead_letter.map((x) => x.id),
    ]);
...
function settleQueueItem(task, ok, errorMessage) {
  return withLock(QUEUE_LOCK_PATH, () => {
    const queue = loadQueue();
    if (ok) {
      writeJsonAtomic(QUEUE_PATH, queue);
...
function applyPatch(task) {
  ...
  if (task.patch?.type === 'next-config-guardrails') {
    ...
    if (!current.includes(target)) {
      return {ok: false, error: 'unsupported next.config.mjs format'};
    }
...
function runPatches(maxPatches) {
  const executed = [];
  const failed = [];
  ...
  for (let i = 0; i < maxPatches; i += 1) {
    ...
    if (result.ok) {
      ...
      executed.push({
        id: task.id,
        title: task.title,
        files: result.files || task.files || [],
        summary: result.summary,
        rollback: task.rollback,
        alignment: task.alignment || 'Unspecified',
      });
    } else {
      settleQueueItem(task, false, result.error || 'patch failed');
      failed.push({id: task.id, title: task.title, error: result.error || 'patch failed'});
    }
  }
...
function runWorkspaceScriptCheck(workspaceDir, workspaceName, script) {
...
function writeUpgrade(id, northStar, evidence, backlog, queueBefore, patchResults, hardening) {
  mkdirp(OUTPUT_DIR);
  ...
  const backlogSection = backlog.map((t) => {
  ...
  const hardeningDetails = hardening.checks.map((c) => `### ${c.check}\n- status: ${c.status}\n- classification: ${c.classification}\n- command: \`${c.command}\`\n- cwd: \`${path.relative(NEBULA_ROOT, c.cwd) || '.'}\`\n\n\`\`\`\n${c.output}\n\`\`\``).join('\n\n');
